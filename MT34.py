...
def test_subset(args):
    ...
    solutions = []  # liste pour toutes les solutions trouvées
    for permuted_subset in permutations(subset):
        ...
        if verify_reliability(truth_table):
            solutions.append(permuted_subset)# solution trouvé
    return solutions # retourne tte les solutions trouvées dans le sous ensemble


def parallel_test_all_combinations(num_cables, start_size, max_solutions):      #remplace le test_all_combinations, en utilissant un pool qui se genère en fonction du nombre de coeur du processeur et alloue chaque sous ensemble de façon equilibré
    """
        Recherche les cablages fiables en parallèle et retourne les max_solutions premières solutions trouvées.
    """
    ...
    solutions = [] # lite globale des solutions
    ...
    for connection_set_size in range(start_size, len(all_possible_connections) + 1):
        ...
        # analyse result pour trouver 1 solution --> changé encollecte des solution trouvées
        for result in results:
            if result: # si solution trouvée
                solutions.extend(result) # ajouter solution trouvée à la liste
                # Si on a atteint le nombre maximum de solutions, arrêter
                if len(solutions) >= max_solutions:
                    return solutions[:max_solutions]

    return solutions  # Retourner toutes les solutions trouvées si moins de max_solutions disponibles
...
if __name__ == "__main__":
    multiprocessing.set_start_method("spawn", force=True) #important pour la compilation
    ...
    max_solutions = 3 # nombre maximum de solutions à trouver

    optimal_connections = parallel_test_all_combinations(num_cables, start_size, max_solutions)

    if optimal_connections:
        print(f"[  End!  ] ({len(optimal_connections)}/{max_solutions}) combinaisons de connexions fiables trouvées:")
        for idx, conn in enumerate(optimal_connections, start=1):
            print(f"Solution #{idx}:")
            for item in conn:
                print(f"  {item}")  # Chaque élément affiché directement en brut
    else:
