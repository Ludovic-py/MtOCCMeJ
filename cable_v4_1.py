...
def conversion_fonction(liste, etape):
    ...
    for j in range(1, len(liste)):
        if j != 1:
            lines.append(f"{indentation}{liste[0]},")
        if isinstance(liste[j], list):
            lines.append(f"max|{conversion_fonction(liste[j], etape + 1)},")
        else:
            lines.append(liste[j])
...
class MainWindow(Canvas):
    def __init__(self):
        ...
        self.padding = 100 # ecart bord D et G
        self.gap_top = 50 # ecart top
        # self.g = 100  # écart au bord gauche et en haut du canvas  ps: pas bien car les proportion left et top sont toujours le mêmes alors que sur un design dynamique c'est pas forcement vrai
        # self.d = 100  # écart au bord droit du canvas

        # Create the canvas
        Canvas.__init__(self, self.root, bg="antique white") # couleur changée de bleu clai je crois car pique les yeux
        ...
        self.root.bind("i", self.clic_droit)
